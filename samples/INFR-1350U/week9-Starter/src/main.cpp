...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader2->LoadShaderPartFromFile("shaders/frag_blinn_phong_textured - Copy.glsl", GL_FRAGMENT_SHADER);
		shader2->Link();
		
		ShaderMaterial::sptr reflectiveMat2 = ShaderMaterial::Create();

		reflectiveMat2->Shader = shader2;
		reflectiveMat2->Set("s_Diffuse", diffuse);
		reflectiveMat2->Set("s_Diffuse2", diffuse2);
		reflectiveMat2->Set("s_Specular", specular);
		reflectiveMat2->Set("u_Shininess", 8.0f);
		reflectiveMat2->Set("u_TextureMix", 0.5f);
		reflectiveMat2->Set("s_Reflection", reflection);
		
		reflectiveMat2->Set("s_Environment", environmentMap);
		reflectiveMat2->Set("u_EnvironmentRotation", glm::mat3(glm::rotate(glm::mat4(1.0f), glm::radians(90.0f),
			glm::vec3(1, 0, 0))));

		reflectiveMat2->Set("u_LightPos", lightPos);
		reflectiveMat2->Set("u_LightCol", lightCol);
		reflectiveMat2->Set("u_AmbientLightStrength", lightAmbientPow);
		reflectiveMat2->Set("u_SpecularLightStrength", lightSpecularPow);
		reflectiveMat2->Set("u_AmbientCol", ambientCol);
		reflectiveMat2->Set("u_AmbientStrength", ambientPow);
		reflectiveMat2->Set("u_LightAttenuationConstant", 1.0f);
		reflectiveMat2->Set("u_LightAttenuationLinear", lightLinearFalloff);
		reflectiveMat2->Set("u_LightAttenuationQuadratic", lightQuadraticFalloff);

		GameObject sceneObj = scene->CreateEntity("scene_geo");
		{
			VertexArrayObject::sptr sceneVao = NotObjLoader::LoadFromFile("Sample.notobj");
			sceneObj.emplace<RendererComponent>().SetMesh(sceneVao).SetMaterial(reflectiveMat2);
