...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
			...
			if (ImGui::CollapsingHeader("Effect controls"))
			{
				...
				*/
				if (activeEffect == 0)
				{
					...
					float intensity = temp->GetBrightness();

					if (ImGui::SliderFloat("Brightness Threshold", &intensity, 0.0f, 1.0f))
					{
						temp->SetBrightness(intensity);
					}
				}
		...
		Projshader->LoadShaderPartFromFile("shaders/Midterm_frag_shader.glsl", GL_FRAGMENT_SHADER);
		Projshader->Link();

		glm::vec3 lightPos2 = glm::vec3(0.0f, 0.0f, 2.0f);
		glm::vec3 lightCol2 = glm::vec3(1.0f, 1.0f, 1.0f);
		...
		Projshader->SetUniform("u_LightPos", lightPos2);
		Projshader->SetUniform("u_LightCol", lightCol2);
		Projshader->SetUniform("u_AmbientCol", ambientCol2);
		...
		grassMat->Shader = Projshader;
		grassMat->Set("s_Diffuse", grass);
		grassMat->Set("s_Specular", 5.0f);
		...
		EnvironmentGenerator::AddObjectToGeneration("models/simplePine.obj", ProjTreeMat, 150,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		EnvironmentGenerator::AddObjectToGeneration("models/simpleTree.obj", ProjTreeMat, 150,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		EnvironmentGenerator::AddObjectToGeneration("models/simpleRock.obj", ProjTreeMat, 40,
		...
		*/
		...
		{
			colorCorrect = &colorCorrectionObject.emplace<ColorCorrection>();
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			*/
			...
			//colorCorrect->ApplyEffect(basicEffect);
