...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		Model = glm::mat4(1.0f);
		keyboard();												//X	    Y     Z
		Model = glm::rotate(Model, glm::radians(rotY), glm::vec3(0.0f, 1.0f, 0.0f)) * glm::rotate(Model, glm::radians(rotX), glm::vec3(1.0f, 0.0f, 0.0f));
		...
		// draw points 0-18 
		glDrawArrays(GL_TRIANGLES, 0, 36); //change to 36 for hands-on
