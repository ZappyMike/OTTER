...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		Shader::sptr Projshader = Shader::Create();
		Projshader->LoadShaderPartFromFile("shaders/vertex_shader.glsl", GL_VERTEX_SHADER);
		Projshader->LoadShaderPartFromFile("shaders/A1_frag_shader.glsl", GL_FRAGMENT_SHADER);
		Projshader->Link();
		...
		bool ambientBool = false;
		bool specularBool = false;
		bool noLighting = false;
		bool allLighting = true;
		bool toonLighting = false;
	
		Projshader->SetUniform("u_AmbientStrength", ambientPow);
		Projshader->SetUniform("u_AmbientLightStrength", lightAmbientPow);
		Projshader->SetUniform("u_LightPos", lightPos);
		Projshader->SetUniform("u_LightCol", lightCol);
		Projshader->SetUniform("u_AmbientCol", ambientCol);
		...
		imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::Checkbox("All lighting(Ambient, Specular and Diffuse)", &allLighting))
			{
				allLighting = true;
			...
			if (ImGui::Checkbox("Only Ambient Lighting", &ambientBool))
			{
				ambientBool = true;
			...
			if (ImGui::Checkbox("Only Specular Lighting", &specularBool))
			{
				specularBool = true;
			...
			if (ImGui::Checkbox("Toon Shading", &toonLighting))
			{
				toonLighting = true;
		...
		Texture2D::sptr diffuse = Texture2D::LoadFromFile("images/sand.jpg");
		Texture2D::sptr diffuse2 = Texture2D::LoadFromFile("images/sand.jpg");
		...
		*/
		...
		ShaderMaterial::sptr ProjMat = ShaderMaterial::Create();
		ProjMat->Shader = Projshader;
		ProjMat->Set("s_Specular", 5.0f);
		ProjMat->Set("u_Shininess", 8.0f);
		ProjMat->Set("s_Diffuse", charTex);

		ShaderMaterial::sptr ProjMat2 = ShaderMaterial::Create();
		ProjMat2->Shader = Projshader;
		ProjMat2->Set("s_Specular", 5.0f);
		ProjMat2->Set("u_Shininess", 8.0f);
		ProjMat2->Set("s_Diffuse", groundTex);
		...
		GameObject obj2 = scene->CreateEntity("monkey_quads");
		{
			...
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material1);
		...
		GameObject Char = scene->CreateEntity("First_Char");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/idle0 (1).obj");
			Char.emplace<RendererComponent>().SetMesh(vao).SetMaterial(ProjMat);
			Char.get<Transform>().SetLocalPosition(-2.0f, -2.0f, 0.0f);
			Char.get<Transform>().SetLocalRotation(90.0f, 0.0f, -45.0f);

			//movement
			auto pathing = BehaviourBinding::Bind<FollowPathBehaviour>(Char);

			pathing->Points.push_back({ 0.0f, 0.0f, 0.0f });
			pathing->Points.push_back({ -2.0f, -2.0f, 0.0f });
			pathing->Speed = 4.0f;
		}
		GameObject Char2 = scene->CreateEntity("Second_Char");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/idle0 (2).obj");
			Char2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(ProjMat);
			Char2.get<Transform>().SetLocalPosition(-2.0f, 2.0f, 0.0f);
			Char2.get<Transform>().SetLocalRotation(90.0f, 0.0f, -135.0f);

			//movement
			auto pathing = BehaviourBinding::Bind<FollowPathBehaviour>(Char2);

			pathing->Points.push_back({ 0.0f, 0.0f, 0.0f });
			pathing->Points.push_back({ -2.0f, 2.0f, 0.0f });
			pathing->Speed = 4.0f;
		}
		GameObject Char3 = scene->CreateEntity("Third_Char");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/idle0 (3).obj");
			Char3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(ProjMat);
			Char3.get<Transform>().SetLocalPosition(2.0f, -2.0f, 0.0f);
			Char3.get<Transform>().SetLocalRotation(90.0f, 0.0f, 45.0f);

			//movement
			auto pathing = BehaviourBinding::Bind<FollowPathBehaviour>(Char3);

			pathing->Points.push_back({ 0.0f, 0.0f, 0.0f });
			pathing->Points.push_back({ 2.0f, -2.0f, 0.0f });
			pathing->Speed = 8.0f;
		}
		GameObject Char4 = scene->CreateEntity("Fourth_Char");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/idle0 (4).obj");
			Char4.emplace<RendererComponent>().SetMesh(vao).SetMaterial(ProjMat);
			Char4.get<Transform>().SetLocalPosition(2.0f, 2.0f, 0.0f);
			Char4.get<Transform>().SetLocalRotation(90.0f, 0.0f, 135.0f);

			//movement
			auto pathing = BehaviourBinding::Bind<FollowPathBehaviour>(Char4);
		...
		GameObject ground = scene->CreateEntity("Ground");
		{
			...
			ground.emplace<RendererComponent>().SetMesh(vao).SetMaterial(ProjMat2);
			ground.get<Transform>().SetLocalPosition(0.0f, 0.0f, 0.0f);
			ground.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
		}
