...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		Texture2D::sptr projgrass = Texture2D::LoadFromFile("images/MidtermGrass.jpg");
		...
		grassMat->Set("s_Diffuse", projgrass);
		...
		*/
		...
		GameObject shadowBufferObject = scene->CreateEntity();
		{
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			glm::mat4 lightProjectionMatrix = glm::ortho(-20.0f, 20.0f, -20.0f, 20.0f, -30.0f, 30.f);
